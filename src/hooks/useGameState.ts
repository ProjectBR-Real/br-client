...
export const useGameState = (gameId: string | null, onSerialEvent?: (event: string) => void) => {
  ...
  useEffect(() => {
    ...
    const handleSerialDataLocal = async (data: string, gId: string) => {
      ...
      if (line.startsWith("ITEM:")) {
        ...
        // Notify via callback
        onSerialEvent?.(`[SERIAL] Item detected: ITEM:${itemType.toUpperCase()}`);
        ...
        try {
          ...
          onSerialEvent?.(`[SERIAL] ✗ Error: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
  ...
  const connectSerial = async () => {
    try {
      ...
      onSerialEvent?.(`[SERIAL] ✗ Connection failed: ${err instanceof Error ? err.message : String(err)}`);
